...
Function Enable_Start_Event_Logs
{
    ...
    foreach ($each_log in $global:event_list)
    {
        ...
        if($each_log_settings.IsEnabled -eq $false)
        {
            ...
            {
                If ($global:verbose_output){Write-Host "We are change size and enabled $each_log logs" -ForegroundColor Red}
                wevtutil.exe set-log $each_log  /enabled:true /ms:102400000 /q:true
            }
            else
            {
                If ($global:verbose_output){Write-Host "We are enableing" $each_log -ForegroundColor Red}
                wevtutil.exe set-log $each_log /enabled:true /q:true
            }
        }
        else
        {
            If ($global:verbose_output){Write-Host "We see" $each_log "is enabled" -ForegroundColor Red}
        }
    }
}
...
Function Stop_NetTrace_Get_NetInfo
{
    ...
    #Powershell.exe -ExecutionPolicy RemoteSigned -File $global:current_Folder"\Test-DeviceRegConnectivity.ps1" | Out-Null
    #Write-Host "Starting "$global:current_Folder"\Test-DeviceRegConnectivity.ps1"
    Powershell.exe -ExecutionPolicy RemoteSigned -File $global:current_Folder"\Test-DeviceRegConnectivity.ps1" | Out-File -FilePath $global:full_net_folder"Test_DeviceRegConnectivity_Output.txt" 2>&1 | Out-Null
...
Function Stop_Event_Logs
{
    ...
    foreach ($each_log in $global:event_list)
    {
        ...
        if ($global:event_log_disabled_by_default.Contains($each_log))
        {
            if ($each_log.Contains("CAPI2"))
            {
                If ($global:verbose_output){Write-Host "We are handling CAPI2 to" $event_file_path -ForegroundColor Red}
                wevtutil.exe set-log $each_log /enabled:false /ms:1052672 /q:true | Out-Null
                wevtutil.exe export-log $each_log $event_file_path /overwrite:true | Out-Null
            }
            else
            {
                If ($global:verbose_output){Write-Host "We are disabling" $each_log "and save to" $event_file_path -ForegroundColor Red}
        ...
        {
            If ($global:verbose_output){Write-Host "We see $each_log is enabled and save to" $event_file_path -ForegroundColor Red}
            wevtutil.exe export-log $each_log $event_file_path /overwrite:true | Out-Null
        }
    }
...
Function Get_Reg
{
    If ($global:verbose_output){Write-Host "We are in Get-Reg" -ForegroundColor Red}
    #Write-Host " Collecting registry keys information..." -ForegroundColor Blue
    ...
    #Write-Host " Done collecting registry keys information...`n" -ForegroundColor Blue
}
...
Function Get_Certs
{
    #Write-Host " Collecting certificates information..." -ForegroundColor Blue
    certutil -v -silent -store ROOT | Out-File $global:full_cert_folder"CERT_Root.txt"
    certutil -v -silent -store -user ROOT | Out-File $global:full_cert_folder"CERT_Root_User.txt"
    certutil -v -silent -store CA | Out-File $global:full_cert_folder"CERT_CA.txt" 
    certutil -v -silent -store AUTHROOT | Out-File $global:full_cert_folder"CERT_AUTHROOT.txt"
    certutil -v -silent -store -enterprise ROOT | Out-File $global:full_cert_folder"CERT_Enterprise_Root.txt"
    certutil -v -silent -store -enterprise NTAUTH | Out-File $global:full_cert_folder"CERT_Enterprise_NTAuth.txt" 
    certutil -v -silent -store -grouppolicy ROOT | Out-File $global:full_cert_folder"CERT_GPO_ROOT.txt" 
    certutil -v -silent -store MY | Out-File $global:full_cert_folder"CERT_Machine_MY.txt"
    certutil -v -silent -store -user MY | Out-File $global:full_cert_folder"CERT_User_MY.txt"
    certutil -v -silent -scinfo | Out-File $global:full_cert_folder"CERT_smart_card_info.txt" # Displays information about the smart card.
    certutil -tpminfo | Out-File $global:full_cert_folder"CERT_TPM_Info.txt"
    certutil -v -silent -user -key -csp | Out-File $global:full_cert_folder"CERT_MS_Passport_Key.txt"
    certutil -v -silent -user -store my | Out-File $global:full_cert_folder"CERT_MS_Passport_Provider.txt"
    #Write-Host " Done collecting certificates information...`n" -ForegroundColor Blue
}

Function Get_SCP
{
    If ($global:verbose_output){Write-Host "We are in Get SCP" -ForegroundColor Red}
    #Write-Host " Collecting SCP information..." -ForegroundColor Blue
...
{
    If ($global:verbose_output){Write-Host "We are in Get all other info" -ForegroundColor Red}
    #Write-Host " Collecting all other information..." -ForegroundColor Blue

    #dsregcmd
    dsregcmd.exe /status > $global:full_folder"\dsregcmd_status_admin_end.txt" 2>&1 | Out-Null
    dsregcmd.exe /debug > $global:full_folder"\dsregcmd_debug_admin_end.txt" 2>&1 | Out-Null
    dsregcmd.exe /status_old > $global:full_folder"\dsregcmd_status_old_admin_end.txt" 2>&1 | Out-Null
    dsregcmd.exe /ListAccounts > $global:full_folder"\dsregcmd_ListAccounts_admin_end.txt" 2>&1 | Out-Null
    dsregcmd.exe /RunSystemTests > $global:full_folder"\dsregcmd_RunSystemTests_admin_end.txt" 2>&1 | Out-Null
